...
while Team_1_beat < 10000:
    ...
    #Variable assignment
        ...
        for i in range(27):  # repeats for every possible selection in place
            if places[i-1][-1] == "!" and places[i][-1] == "!" and places[i+1][-1] == "!" and i != 0 and i != 8 and i != 9 and i != 17 and i != 18 and i != 26: #chacks if the pieces bside are activated and makes sure it ony checks peices that can have a piece on both sides
                points += 5
                #adds an L on the end so these pieces and be used for more than one link
                places[i - 1] += "L"
                places[i] += "L"
                places[i + 1] += "L"
    ...
    if Team1 < Team2:
           print((bot(bot4_piece, bot4_level) + bot(bot5_piece, bot5_level) + bot(bot6_piece, bot6_level))-(bot(bot1_piece, bot1_level) + bot(bot2_piece, bot2_level) + bot(bot3_piece, bot3_level)))
           print("team 1 beat")
           Team_1_beat = 0
           ...
           print("bot 4 piece is " + str(bot4_piece))
           print("bot 4 level is " + str(bot4_level))
           print("bot 5 piece is " + str(bot5_piece))
           print("bot 5 level is " + str(bot5_level))
           print("bot 6 piece is " + str(bot6_piece))
           print("bot 6 level is " + str(bot6_level))

    else:
        Team_1_beat += 1
        print((bot(bot1_piece,bot1_level)+bot(bot2_piece,bot2_level)+bot(bot3_piece,bot3_level)) - (bot(bot4_piece,bot4_level)+bot(bot5_piece,bot5_level)+bot(bot6_piece,bot6_level)))
        bot4_piece = random.randint(1, 3)
        bot4_level = random.randint(1, 3)
        bot5_piece = random.randint(1, 3)
        bot5_level = random.randint(1, 3)
